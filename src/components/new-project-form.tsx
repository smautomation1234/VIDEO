...
import { VISUAL_STYLE_OPTIONS } from "@/features/project/visual-style";
...
export function NewProjectForm() {
  ...
  return <form className="panel project-form" onSubmit={submit}>
    <div className="section-title"><h2>New automated Reel</h2><span>Gemini → Omni Flash</span></div>
    ...
    <div className="two-columns">
      <label>Resolution<select value="720p" disabled><option>720p · Omni Flash</option></select></label>
      <label>Visual style<select value={String(form.style)} onChange={(e) => setForm({ ...form, style: e.target.value })}>{VISUAL_STYLE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
    </div>
