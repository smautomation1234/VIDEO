...
export function StudioClient({ initial }: { initial: StudioPayload }) {
    ...
    try {
      const prompts = Object.fromEntries(
        reviewClips.map((clip) => [
          clip.key,
          editedPrompts[clip.key] !== undefined
            ? editedPrompts[clip.key]
            : clip.prompt,
        ]),
      );
  ...
  const reviewClips = useMemo(() => {
    if (data.project.mode === "edit_video") {
      ...
      const fullScript = data.project.prompt_plan?.full_script || data.project.raw_post;
      return planClips.map((c) => ({
        key: String(c.clip_number),
        clip_number: c.clip_number,
        duration_seconds: c.duration_seconds,
        spoken_line: c.spoken_line,
        prompt: materializeFullScript(c.prompt, fullScript),
      }));
    }
  }, [data.project.mode, data.project.prompt_plan, data.project.raw_post, data.clips]);
        ...
        <div className="review-container">
              ...
              return (
                <div key={clip.key} className="prompt-card">
                  ...
                  <textarea
                    ...
                    placeholder="Enter visual generation prompt for Omni Flash..."
                  />
                </div>
              );
            })}
          ...
          <div className="approve-bar">
            <div>
              ...
              <span style={{ fontSize: "12px", color: "#71717A" }}>Gemini Omni Flash will generate takes for all {reviewClips.length} clips.</span>
            </div>
            <button
              ...
              disabled={busy !== null}
            >
              {busy === "approving" ? "Initializing Omni Flash..." : "Approve & Generate Takes"}
            </button>
          </div>
        </div>
      </section>
          ...
          <header className="generating-header">
            <div>
              ...
              <p style={{ fontSize: "13px", color: "#71717A", margin: "4px 0 0 0" }}>
                Gemini Omni Flash is processing visual scene instructions and generating clips.
              </p>
            </div>
